 ...
 */
   ...
   ATL_hemmR_rec(syp, Mblks, mr, nbL, rL, A, B, C);
   ATL_hemmR_rec(syp, Mblks, mr, nbR, rR, A+(syp->lda+1)*(nL SHIFT), B01, C01);
   if (syp->uplo == AtlasLower)
   {
      A10 = A + (nL SHIFT);
...
SERIAL:
   ...
   ATL_goparallel(p, Mjoin(PATL,DoWorkHEMM), symms, NULL);
}
