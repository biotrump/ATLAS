...
void PrintUsage(char *name, int ierr, char *flag)
{
   ...
   fprintf(stderr,
           "   -o <fname> : output filename [<pre>MVNK.sum]\n");
   fprintf(stderr,
           "   -i <fnam> : kernel index file [MVNCASES/<pre>cases.idx]\n");
   fprintf(stderr,
 ...
 */
   ...
   kp->exflags = DupString("MVNKdir=EXTDIR");
   kp->MU = kp->minM = CL;
   kp->NU = kp->minN = NUs[nnu-1];
   kp->TA = AtlasNoTrans;
   SET_FLAG(kp->flag, MVF_AXPYBASED, 1);
      ...
      for (r1p=r1b; r1p; r1p = r1p->next)
      {
         if (r1p->ID >= 900000 && r1p->ID < 1000000)
         {
            if (r1p->exflags)
            {
               ...
               r1p->exflags = DupString("MVNKdir=EXTDIR");
         }
      }
      if (r1b->mflop[3] <= 0.0)
      {
